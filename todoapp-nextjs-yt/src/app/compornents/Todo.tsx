...
const Todo = ({ todo }: TodoProps) => {
  const ref = useRef<HTMLInputElement>(null); //useRef=簡単にHTML属性にアクセスできるためのHooks

  const [isEditing, setIsEditing] = useState(false);
  const [editedTaskTitle, setEditedTaskTitle] = useState(todo.text);
  const handleEdit = async () => {
    setIsEditing(true); //編集ボタンを押した際、編集中か否かを判断できるように
  };

  const handleSave = async () => {
    await editTodo(todo.id, editedTaskTitle);
    setIsEditing(false); //編集中ではないから、false
  };
    ...
    >
      {isEditing ? (
        <input
          ref={ref}
          type="text"
          className="mr-2 py-1 px-2 rounded border-gray-500 border"
          value={editedTaskTitle}
          onChange={
            (e: React.ChangeEvent<HTMLInputElement>) =>
              setEditedTaskTitle(e.target.value) //入力フォームに文字列を打ち込めるようになる
          }
        />
      ) : (
        <span>{todo.text}</span>
